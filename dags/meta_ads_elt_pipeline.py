...
AIRBYTE_GS_CONNECTION_ID = Variable.get("AIRBYTE_GS_META_ADS_POSTGRES_CONNECTION_ID")
...
def meta_ads_elt_pipeline():
    ...
    @task_group(group_id="airbyte_sync_gs")
    def airbyte_sync_group_gs():
        """Grupo de tarefas para execução da sincronização Airbyte (GS)"""

        trigger_sync_gs = HttpOperator(
            task_id="trigger_connection_sync_gs",
            http_conn_id="airbyte",
            endpoint="/api/v1/connections/sync",
            method="POST",
            headers={
            ...
            data=json.dumps({"connectionId": AIRBYTE_GS_CONNECTION_ID}),
            response_check=lambda response: (
                response.json().get("job", {}).get("status") == "running"
            ),
            response_filter=lambda response: response.json(),
        ...
        sync_monitor_gs = HttpSensor(
            task_id="monitor_sync_status_gs",
            http_conn_id="airbyte",
            endpoint=(
                "/api/public/v1/jobs/" 
                "{{ ti.xcom_pull(task_ids='airbyte_sync_gs.trigger_connection_sync_gs', key='return_value')['job']['id'] }}"
            ),
            method="GET",
            headers=generate_airbyte_auth(),
            response_check=lambda response: response.json()["status"] == "succeeded",
        ...
        trigger_sync_gs >> sync_monitor_gs
    ...
    def dbt_transform_group():
        ...
        return DbtTaskGroup(
            ...
            render_config=RenderConfig(
                select=["path:models/staging/meta_ads"]
            ),
            operator_args={
                "install_deps": True,
                "full_refresh": True
    ...
    [airbyte_sync_group(), airbyte_sync_group_gs()] >> dbt_transform_group()
